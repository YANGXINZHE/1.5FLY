...
void MPU6050_CalOff_Acc(void)//使能加速度计校准
{
	ACC_OFFSET_OK = 0;
}
void MPU6050_CalOff_Gyr(void)//使能陀螺仪校准
{
	GYRO_OFFSET_OK = 0;
}
/**************************实现函数********************************************
...
void MPU6050_Dataanl(T_int16_xyz *data_tempacc,T_int16_xyz *data_tempgyr)
{
	...
	acc_x=((((int16_t)mpu6050_buffer[0]) << 8) | mpu6050_buffer[1]) - ACC_OFFSET.X;//去掉校准数据
	...
	//????ADC
	gyr_x=((((int16_t)mpu6050_buffer[8]) << 8) | mpu6050_buffer[9]) - GYRO_OFFSET.X;//去掉校准数据
...
{
  i2cdevRead(I2Cx,devAddr,MPU6050_RA_ACCEL_XOUT_H,14,mpu6050_buffer);
}
/**************************实现函数********************************************
...
{
    uint8_t byte;
    i2cdevReadByte(I2Cx, dev, reg, &byte);
    byte = (data != 0) ? (byte | (1 << bitNum)) : (byte & ~(1 << bitNum));
    i2cdevWriteByte(I2Cx, dev, reg, byte);
}
/**************************实现函数********************************************
...
void IICwriteBits(uint8_t dev,uint8_t reg,uint8_t bitStart,uint8_t length,uint8_t data)
{
  ...
  if ((status = i2cdevReadByte(I2Cx, dev, reg, &byte)) == TRUE)
  {
      ...
      status = i2cdevWriteByte(I2Cx, dev, reg, byte);
  }
	//return status;
}
/**************************实现函数********************************************
...
*******************************************************************************/
void MPU6050_Init(I2C_TypeDef *i2cPort)
{
  I2Cx = i2cPort;
