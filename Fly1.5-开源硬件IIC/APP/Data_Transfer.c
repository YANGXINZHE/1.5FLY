...
u8 data_to_send[50];
void Nrf_Check_Event(void)//24L01事件
{
...
void Data_Receive_Anl(__IO u8 *data_buf,u8 num)//数据接收解码
{
	vs16 rc_value_temp;
	u8 sum = 0;
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	{   
		...
		if(*(data_buf+4)==0X01)//加速度计补偿使能
			MPU6050_CalOff_Acc();
		if(*(data_buf+4)==0X02)//陀螺仪补偿使能
			MPU6050_CalOff_Gyr();
		if(*(data_buf+4)==0X03)//加速度计补偿使能，陀螺仪补偿使能
...
//			MS5611_CalOffset();
	...
	if(*(data_buf+2)==0X03)//遥控功能字
	{
		...
		Rc_Fun(&Rc_D,&Rc_C);//不使用遥控器时用这个函数对姿态控制
	}
#endif
	...
	if(*(data_buf+2)==0X16)								//OFFSET  角度微调
	{
...
#ifndef DATA_TRANSFER_USE_USART     //如果未定义DATA_TRANSFER_USE_USART  就执行NRF_TxPacket(data_to_send,_cnt);
	NRF_TxPacket(data_to_send,_cnt);
#else           //否则  就执行Uart2_Put_Buf(data_to_send,_cnt);或Uart1_Put_Buf(data_to_send,_cnt);
	if(DEBUG_EN== TRUE)
	    Uart1_Put_Buf(data_to_send,_cnt);
	else                               //以上两个都不满足 就执行Uart2_Put_Buf(data_to_send,_cnt);
...
#ifndef DATA_TRANSFER_USE_USART
	NRF_TxPacket(data_to_send,_cnt);
#else           //否则  就执行Uart2_Put_Buf(data_to_send,_cnt);或Uart1_Put_Buf(data_to_send,_cnt);
	if(DEBUG_EN== TRUE)
	    Uart1_Put_Buf(data_to_send,_cnt);
...
void Data_Send_Check(u16 check)//crc校验返回
{
	data_to_send[0]=0xAA;
	data_to_send[1]=0xAA;
	data_to_send[2]=0xF0;
